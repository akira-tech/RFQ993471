...
Template.home.rendered = function () {
    ...
    function parseText(t) {
        tags = {};
        var e = {};
        t.split(false ? /\n/g : wordSeparators).forEach(function (t) { //d3.select("#per-line").property("checked")
    ...
    !function (t) {
        function e() {
            function t(t, n, a) {
                for (var r, o, s, l = ([
                    {x: 0, y: 0},
                    {x: e[0], y: e[1]}
                ], n.x), i = n.y, h = Math.sqrt(e[0] * e[0] + e[1] * e[1]), d = v(e), f = Math.random() < .5 ? 1 : -1, p = -f; (r = d(p += f)) && (o = ~~r[0], s = ~~r[1], !(Math.min(o, s) > h));)if (n.x = l + o, n.y = i + s, !(n.x + n.x0 < 0 || n.y + n.y0 < 0 || n.x + n.x1 > e[0] || n.y + n.y1 > e[1] || a && u(n, t, e[0]) || a && !c(n, a))) {
            ...
            var e = [256, 256], d = n, p = a, y = r, g = o, x = s, v = h, m = [], w = 1 / 0, b = d3.dispatch("word", "end"), z = null, C = {};
            return C.start = function () {
                ...
                var a = f((e[0] >> 5) * e[1]), r = null, o = m.length, u = -1, c = [], h = m.map(function (t, e) {
                    return{text: d.call(this, t, e), font: p.call(this, t, e), rotate: g.call(this, t, e), size: ~~y.call(this, t, e), padding: s.call(this, t, e)}
                }).sort(function (t, e) {
        ...
        function l(t, e, n) {
            if (!t.sprite) {
                ...
                for (n--; ++n < s;) {
                    ...
                    var l = w.measureText(t.text + "m").width * v, u = t.size << 1;
                    if (t.rotate) {
                        var i = Math.sin(t.rotate * y), c = Math.cos(t.rotate * y), h = l * c, d = l * i, f = u * c, p = u * i;
                ...
                for (var m = w.getImageData(0, 0, (g << 5) / v, x / v).data, M = []; --n >= 0;) {
                    ...
                    for (var T = 0, k = -1, A = 0; u > A; A++) {
                        for (var C = 0; l > C; C++) {
                            var L = b * A + (C >> 5), I = m[(r + A) * (g << 5) + (a + C) << 2] ? 1 << 31 - C % 32 : 0;
        ...
        var p, y = Math.PI / 180, g = 64, x = 2048, v = 1;
        if ("undefined" != typeof document)p = document.createElement("canvas"), p.width = 1, p.height = 1, v = Math.sqrt(p.getContext("2d").getImageData(0, 0, 1, 1).data.length >> 2), p.width = (g << 5) / v, p.height = x / v; else {
    ...
    var fill = d3.scale.category20b(), w = 960, h = 600, words = [], max, scale = 1, complete = 0, keyword = "", tags, fontSize, maxLength = 30, fetcher, statusText = d3.select("#status"), layout = d3.layout.cloud().timeInterval(10).size([w, h]).fontSize(function (t) {
        ...
        }).on("word", progress).on("end", draw), svg = d3.select("#vis").append("svg").attr("width", w).attr("height", h), background = svg.append("g"), vis = svg.append("g").attr("transform", "translate(" + [w >> 1, h >> 1] + ")");
...
Template.home.events({
    ...
    'click #download-png': downloadPNG,
